                    ...
                    <el-form ref="editForm" label-width="150px" :model="model" :rules="rules" status-icon>
                        ...
                        <el-form-item label="工单状态：">
                            <el-select v-model="model.ticketStatus">
                                <el-option label="待分配" :value="0"></el-option>
                                <el-option label="待接单" :value="1"></el-option>
                                <el-option label="派单" :value="2"></el-option>
                                <el-option label="已完成" :value="3"></el-option>
                                <el-option label="已关闭" :value="4"></el-option>
                                <el-option label="已作废" :value="5"></el-option>
                            </el-select>
                        ...
                        <el-form-item label="报修人：" prop="repairPerson">
                            <el-input style="width: 300px" placeholder="请输入地址信息..." v-model.trim="model.repairPerson"></el-input>
                        </el-form-item>

                        <el-form-item label="报修位置：" prop="adds">
...
export default {
        ...
        return {
            ...
            rules: {
                ticketNumber: [
                    ...
                    { min: 2, max: 30, message: "* 工单编号长度为2~30", trigger: "blur" }
                ],
                repairPerson: [
                    { required: true, message: "* 报修人不能为空..", trigger: "blur" },
                    { min: 2, max: 30, message: "* 报修人长度为2~30", trigger: "blur" }
                ],
                adds: [
                    { required: true, message: "* 报修地址不能为空..", trigger: "blur" },
                    { min: 2, max: 30, message: "* 报修地址长度为2~30", trigger: "blur" }
                ]
            },
            // 导出excel表格的代码---start
            title: "123表格",
    ...
    methods: {
        ...
        async getData() { //重新获取数据 -- 在这里发送数据统计的Ajax
            const { list, total } = await this.$api.warrantyManage.getData(this.search); // 这行才是核心模拟从服务器拿数据的请求
            this.list = list;
            this.total = total;
            this.workTotal = await this.$api.warrantyManage.workTotal();
            // 将数据列表中的数字转换为对应的中文字符
            this.list = await this.$api.warrantyManage.numToString({data: this.list});
            // 更新详细表单数据
        ...
        async deleteHandler(id) { // 删除所点的数据
            ...
            // 向服务器发送删除请求
            await this.$api.warrantyManage.del(id);
            this.getData();// 重新获取数据
        ...
        async BulkDeleteHandler() { // 批量删除数据
            ...
            // 向服务器发送批量删除请求
            await this.$api.warrantyManage.bulkdelete({ ids: this.selectIds });
        ...
        beginAdd() {
            // 回显数据
            this.model = { id: 0, ticketNumber: "", ticketStatus: 0, ticketCategory: 0, repairPerson: "", adds: "", source: 0, time: "", sendPerson: "", orderTaker: 0, view: "" }; // 表单关联数据回到初始状态
            this.$refs.editForm?.resetFields();  // 将表单重置
            this.$nextTick(() => this.isEditing = true);  // 在下一个时间片打开dialoag
        },
        // 开始更新数据的处理函数
        async beginUpdata(id) {
            this.model = await this.$api.warrantyManage.getModel(id) // 回显数据
            this.$refs.editForm?.resetFields(); // 将表单重置
            this.$nextTick(() => this.isEditing = true);  // 在下一个时间片打开dialoag
        },
        async save() {
            ...
            if (this.model.id === 0) {
                // 根据ID是否为0来判断是新增还是更新
                await this.$api.warrantyManage.add(this.model);
                this.currentPage = 1;
            } else {
                await this.$api.warrantyManage.update(this.model);
                this.getData();
            }
