        ...
        <el-dialog align="left" title="—— 编辑列表 ——" :visible="isEditing" :show-close="false" width="700px"
            ...
            <el-form ref="editForm" label-width="150px" :model="model" :rules="rules" status-icon>
                ...
                <el-form-item label="人脸照片：">
                    <el-upload :headers="uploadHeaders" name="img" action="http://localhost:2999/personnelManage/preupload"
                        accept="image/png,image/jpeg" :before-upload="beforeUploadHandler" :on-success="successHandler"
                        list-type="picture-card" :file-list="imgarr">
                        + 上传头像
                    </el-upload>
                </el-form-item>
                ...
                <el-form-item label="认证状态">
                    <el-select style="width: 300px" v-model="model.conType">
...
export default {
    ...
    data() {
        return {
            ...
            show: false,
            uploadHeaders: {
                // 文件上传的请求头，包含Authorization字段用于携带token
            ...
            model: { id: 0, name: "", tel: "", sex: -1, adds: "", idCard: "", idType: -1, time: "", img: "", vPeriod: "", approvalStatus: 0, conType: 0, tmpFileName: "" },
            isEditing: false,
            // rules: {
            //     name: [
    ...
    methods: {
        beforeUploadHandler(file) {
            ...
            if (!isJPG && !isPNG) {
                this.$message.error('上传头像图片只能是 JPG 格式 或者 PNG格式!');
            }
            if (!isLt2M) {
                this.$message.error('上传头像图片大小不能超过 2MB!');
            }
            return (isJPG || isPNG) && isLt2M;// 返回是否符合上传条件的布尔值
        },
        successHandler(response, file, fileList) {
            // 获取预上传到服务器后的监时文件的名字
            this.model.tmpFileName = response.data;
            // 保持只显示一个上传文件
        ...
        async getData() { //重新获取数据
            const { list, total } = await this.$api.personnelManage.userrealoverrule(this.search); // 这行才是核心模拟从服务器拿数据的请求
            this.list = list;// 更新用户实名认证被驳回的列表数据
            this.total = total;// 更新总数据条数
            this.list = await this.$api.personnelManage.numToString({ data: this.list });// 将数字转换成对应的字符串
        ...
        async beginUpdata(id) {
            ...
            // 更新图片数组
            this.imgarr = [{name: this.model.img, url: this.model.img}];
