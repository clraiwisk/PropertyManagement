...
<template>
    <div style="position: relative;">
        ...
        <el-table :data="list" @selection-change="selectionChangeHandler">
            ...
            <el-table-column prop="time" label="迁出时间" width="200" align="center">
                <template slot-scope="{row}">
...
export default {
    ...
    methods: {
        ...
        //重新获取数据
        async getData() { 
            const { list, total } = await this.$api.personnelManage.moveout(this.search); // 这行才是核心模拟从服务器拿数据的请求
        ...
        async cancel() {
            ...
            this.isEditing = false;// 隐藏编辑对话框
        },
        // 刷新数据
        refresh() {
            this.getData();// 刷新数据，重新获取数据
        },
        // 打开编辑对话框并回显数据
