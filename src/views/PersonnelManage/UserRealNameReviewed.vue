...
export default {
    ...
    methods: {
        ...
        // async toMove() {
        //     await this.$confirm("您真的要将该户主迁出吗？", "警告", { type: "warning" });
        //     console.log(1111)
        //     await this.$api.personnelManage.tomove(this.model.id);
        //     console.log(2222)
        //     this.getData();
        //     console.log(3333)
        ...
        async getData() { //重新获取数据
            const { list, total } = await this.$api.personnelManage.userrealchecked(this.search); // 这行才是核心模拟从服务器拿数据的请求
            this.list = list; // 更新用户实名认证已审核通过的列表数据
